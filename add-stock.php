<?php 
   ...
   if($_SERVER['REQUEST_METHOD'] == 'POST')
  {
    ...
    if(isset($_POST['submit']))
    {
        ...
        $sql2 ="INSERT INTO bill_stock (bill_no, supplier, created_date) VALUES ('$bill_no', '$supplier', '$date')";
          ...
          <div class="card">
             ...
             <div class="row">
               <div class="form-group col-lg-6">
                ...
                <?php
                    $bill_sql = mysqli_query($conn,"SELECT bill_no FROM bill_stock ORDER BY `id` DESC LIMIT 1");
                    $rowdata1 = mysqli_fetch_assoc($bill_sql);
                    $rows = mysqli_num_rows($bill_sql);
                    if($rows == 0){
                      $bill = 101;
                    }else{
                      $pre_bill = $rowdata1['bill_no'];
                      $bill = $pre_bill + 1;
                    }
                ?>
                <input type="text" readonly value="<?php echo $bill ?>" class="form-control form-control-rounded" id="" name="bill_no" placeholder="Supplier">
               </div>

               <div class="form-group col-lg-6">
                <label for="">Suplier</label>
                <select required class="form-control form-control-rounded" id="" name="supplier">
                  <option>Select Suplier</option>
                  <?php 
                      $sql_sup = "SELECT * FROM suplier";
                       $result1 = $conn->query($sql_sup);
                      while($row1 = $result1->fetch_assoc()) 
                      {
                        echo "<option value='".$row1['id']."'>".$row1['suplier_name']."</option>";
                      }
